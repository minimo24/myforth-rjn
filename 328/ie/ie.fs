\ ie.fs -- incremental encoder -- 160504rjn
\
\ 
0 [if] ---------------------------[ Notes ]------------------------------------
\ 
...
:m ?p&c   \ save prev A&B to N, move to upper nibble, update current A&B bits in lower
...
: /encoder  
      ...
      0 p&c ldi,  cura curb ;
: @encoder  ( - d )  'ecount #, cli, |@ sei,  'ecount1 #, cli, |@ sei,  ; 
\ 
\ 
0 [if] \ --- testing
\ 
