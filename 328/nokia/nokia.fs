\ nokia.fs -- Nokia graphics lcd driver -- 160620rjn
\  
\   
...
-: ?0nud.  ( d -)   \ can't use + because of carry
   2dup negate |+ if drop <# #s #> #ntype ; then drop drop drop s" 0" ntype ;
\ 
\ note: Entering caps in a hex number results in bad display.
\       For example, try entering "hex 00AB nh." vs. "hex 00ab nh."
: nh.   ( n)  depth if drop ?0nh.  ; then drop ;
: nud.  ( d)  depth if drop ?0nud. ; then drop ;
: nu.   ( n)  0 #, nud. ;
\  
0 [if] \ test
...
-: ?0bud.  ( d -)   \ can't use + because of carry
   2dup negate |+ if drop <# #s #> #btype ; then drop drop drop s" 0" btype ;
\ 
: bh.   ( n)  depth if drop ?0bh.  ; then drop ;
: bud.  ( d)  depth if drop ?0bud. ; then drop ;
: bu.   ( n)  0 #, bud. ;
\  
0 [if] \ test
...
Date     By    Description
======   ===   ================================================================
160620   rjn   added pattern for demo
160617   rjn   changed names (e.g., !ncon vs. !contrast)
160613   rjn   set backlight from eeprom
160608   rjn   leading zero supression works on both normal and big numeric
               display.
160606   rjn   big characters work, including character wrap.  Numeric output
               still not working.
160528   rjn   verified backlight command, corrected colon definition errs.
160527   rjn   first hack with non-inverting drivers
160517   rjn   initial version cloned from SPI
